...
class FhenixAdapter extends base_1.BaseChainAdapter {
    constructor() {
        super(...arguments);
        this.chain = types_1.Chain.FHENIX;
        this.nativeCurrency = 'tFHE';
        this.rpcUrl = '';
    ...
    getAddress(publicKey) {
        const hash = (0, crypto_1.createHash)('keccak256').update(publicKey.slice(1)).digest();
    ...
    async signTransaction(tx, privateKey) {
        ...
        const txHash = (0, crypto_1.createHash)('keccak256')
    ...
    async createHTLC(params) {
        ...
        const swapId = (0, crypto_1.createHash)('sha256')
    ...
    async createEncryptedHTLC(params) {
        this.ensureInitialized();
    ...
    async fheEncrypt(value) {
        // Use Fhenix FHE library to encrypt
        // This is a placeholder - actual implementation uses @fhenixprotocol/fhenix.js
        return '0x' + (0, crypto_1.createHash)('sha256').update(value.toString()).digest('hex');
    }
    encodeHTLCInit(swapId, params) {
    ...
    encodeEncryptedHTLCInit(params, encryptedAmount) {
        return '0x' +
    ...
    decodeHTLCStatus(result) {
        ...
        let state = types_1.HTLCState.LOCKED;
        if (withdrawn)
            state = types_1.HTLCState.CLAIMED;
        else if (refunded)
            state = types_1.HTLCState.REFUNDED;
        else if (timelock < Date.now() / 1000)
